...
class ChatbotUI:
    ...
    def load_existing_vectorstore(self) -> Optional[Chroma]:
        ...
        try:
            if self.config.VECTOR_DB_DIR.exists() and any(self.config.VECTOR_DB_DIR.iterdir()):
                vectorstore = Chroma(
                    persist_directory=str(self.config.VECTOR_DB_DIR),
    ...
    def process_uploaded_file(self, uploaded_file):
        ...
        try:
            ...
            if self.processor.is_file_processed(file_path):
                ...
                st.session_state.vectorstore = self.load_existing_vectorstore()
                if st.session_state.vectorstore is None:
                    # If loading fails, reprocess the file
                    st.warning("Could not load existing vectorstore. Reprocessing file...")
                    self._process_new_file(file_path, uploaded_file)
            else:
                self._process_new_file(file_path, uploaded_file)
    ...
    def _process_new_file(self, file_path: Path, uploaded_file):
